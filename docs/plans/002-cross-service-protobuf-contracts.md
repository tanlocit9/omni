...
| Telegram deduplication is process-local                             | Operational risk      | Restarts clear the cache and multiple Platform replicas can still deliver duplicates.                            | Treat the current cache as rate limiting only. P8-I3 owns durable delivery identity, retry state, provider message tracking, and cross-instance idempotency.                                                                                                                                           |
