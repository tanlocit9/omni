...
@Slf4j
@Component
public class SectorTransitionAnalyzeJobProducer extends JobProducer {

    private final SymbolRepository symbolRepository;

    @Value("${kafka.topics.topic-sector-transition-analyze}")
    private String topic;

    public SectorTransitionAnalyzeJobProducer(
    ...
    protected List<KafkaMessage> buildMessages(
        ...
        log.info("Running Sector Transition analysis universe={} focus={} sectorLevel={} strategy={} timeframe={} evaluationDate={} horizons={} jobId={} executionId={}",
        ...
        return List.of(new KafkaMessage(
                config.strategy(),
                new SectorTransitionAnalyzeJobMessage(
                        job.getId(),
                        child.getId(),
                        jobExecutionHistory.getId(),
    ...
    protected void postPublish(JobDefinition job, Instant now) {
        log.info("Published Sector Transition analysis job [{}] for source [{}]", job.getId(), job.getSource());
    }
}
