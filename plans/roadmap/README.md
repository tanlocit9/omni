...
P3-I1 is the highest-priority unfinished correctness increment and remains `blocked`; its historical failed CI evidence is unchanged. P4-I1 and P4-I2 are re-baselined as completed from merged implementation plus passing Platform/PostgreSQL coverage in CI run #149. Phase 6 keeps its independent product roadmap state. Phase 7 is completed on draft PR #15 at `0d09cbe14719f83d2536573575795171eca6a168`: only explicitly allow-listed active definitions are triggerable, Platform preserves identity/audit, dependency, exact-claim/concurrency, idempotency, outbox, and status semantics, and force, bypass, cancellation, browser-to-Kafka, secrets, and physical paths remain absent. The Jobs surface requires the verified private identity contract, not completion of the unrelated SQL/Saved Query/Dashboard product increments bundled in Phase 6.
