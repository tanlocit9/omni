...
from app.indicators.messages import IndicatorJobMessage, JobStatusMessage, utc_now
...
class IndicatorKafkaService:
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(self._consume_loop(), name="indicator-kafka-consumer")
        _logger.info("Started indicator Kafka consumer topic=%s groupId=%s", topic, group_id)
    ...
    async def _publish_status(self, status: JobStatusMessage) -> None:
        ...
        await self._producer.send_and_wait(
            ...
            key=status.symbolKey.encode("utf-8"),
        )
    ...
    def _build_status(
        ...
        return JobStatusMessage(
            ...
            metaJson={"recordsProcessed": records_processed},
        )
    ...
    def _build_error_status(
        ...
        duration_ms = int((finished_at - started_at).total_seconds() * 1000)
        return JobStatusMessage(
            jobDefinitionId=str(raw.get("jobDefinitionId", "")),
            executionId=str(raw.get("executionId", "")),
            parentExecutionId=raw.get("parentExecutionId"),
            symbolKey=str(raw.get("symbolKey", "")),
            status="ERROR",
            startedAt=started_at,
            finishedAt=finished_at,
