...
@pytest.mark.asyncio
async def test_query_service_returns_authoritative_latest_signal_without_publisher():
    storage = Storage({path("HOSE"): pd.DataFrame([row()])})
    service = LatestSignalQueryService(LatestSignalRepository(Settings(), storage))

    latest = await service.find_latest("hose-acb")

    assert latest is not None
    assert latest.symbol_key == "HOSE-ACB"
    assert latest.signal == "BULLISH"
    assert latest.reason_codes == ["MOMENTUM"]
...
def test_endpoint_returns_completed_signal_data_and_not_found():
    ...
    class Service:
        async def find_latest(self, symbol_key=None):
            return None if symbol_key == "HOSE-NONE" else found

    client = TestClient(app)
    client.app.state.latest_signal_query_service = Service()
    completed = client.post("/v1/signals/notifications/latest?symbolKey=HOSE-ACB")
    ...
    assert completed.json() == {
        ...
        "newSignal": "BULLISH",
        "price": 25000.0,
        "signalDate": "2026-08-29T00:00:00+00:00",
        "reasonCodes": ["MOMENTUM"],
        "score": 4,
        "strategy": "TREND_MOMENTUM_V1",
        "timeframe": "1d",
...
def test_endpoint_returns_503_when_query_unavailable():
    client = TestClient(app)
    if hasattr(client.app.state, "latest_signal_query_service"):
        client.app.state.__delattr__("latest_signal_query_service")
