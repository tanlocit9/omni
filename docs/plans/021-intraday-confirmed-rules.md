...
Status: implementation in progress / local project verification passed 2026-09-12
Dependency: Phase 9 P9-I1 remains `verification_pending`. Owner instruction on 2026-09-11 authorizes this bounded implementation without promoting P9-I1.
...
Analyzer resolves exact-date VCI intraday evidence, derives deterministic facts, applies a confirmation/suppression gate to the unchanged daily candidate, and persists V2 lineage and explanation through the existing signal history and notification flow.
...
MIXED            = every other valid fact combination
...
| BULLISH/BEARISH                         | UNAVAILABLE               | NO_DECISION |
...
4. `(provider, exchange, trading_date, symbol)` resolves one independently READY-addressable partition;
5. intraday immutable publication is READY and reconciliation is `READY` or `WARNING`;
6. the exact intraday object for the requested symbol/date/provider is resolvable;
7. all input `dataVersion` values are recorded before persistence.

No previous-day or other-symbol intraday fallback. Missing/stale intraday yields
`UNAVAILABLE` -> final `NO_DECISION`.
...
1. Keep P9-I1 `verification_pending`; owner verification remains a separate gate.
2. Apply the approved EOD-style intraday identity: `(provider, exchange, trading_date,
symbol)` with a stable `trades.parquet` leaf and one READY pointer per symbol.
3. Republish any object created under the former shared provider/exchange/date READY
   layout. V2 provides no compatibility fallback because a shared pointer cannot prove
   readiness for the requested symbol.
4. Add Analyzer intraday input resolver and pure fact derivation.
5. Add pure intraday confirmation rule.
6. Add explicit V2 confirmed composition while preserving V1 behavior for historical interpretation/tests.
7. Extend signal metadata/storage for `intradayConfirmation` and exact input versions.
8. Add scheduler dependency/order after exact-date intraday READY.
9. Extend Query Service/Console/Telegram rendering for optional intraday explanation without recomputation.
10. Activate V2 for future rows only; historical V1 backfill is a separate owner action.
...
Owner verification was authorized and run on 2026-09-12. Local Analyzer and
Platform project checks passed; external and runtime checks remain unresolved.

| Status  | Check                                                                   | Source evidence / target                                                                                                                      |
| ------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| PARTIAL | P9-I1 owner verification                                                | Local py-common, Ingestor, Analyzer, and Platform project checks passed on 2026-09-12; runtime, PR/commit, and CI evidence remains incomplete |
...
| NOT RUN | Two symbols have independent READY pointers                             | `StockDataPaths.intraday_trades`; `apps/ingestor/app/handlers/intraday_eod.py`; shared path/publication source tests                          |
| NOT RUN | Former shared date-level READY objects require republishing             | `docs/data/002-data-lake.md`; no resolver compatibility fallback                                                                              |
...
| NOT RUN | Intraday rule matrix returns CONFIRM/MIXED/UNAVAILABLE exactly          | `apps/analyzer/tests/test_intraday_signal_confirmation.py`                                                                                    |
...
| PASS    | Relevant Analyzer/Platform Nx targets                                   | 2026-09-12: Analyzer lint/test (131 passed)/build and Platform test/build passed; Query/Console were unchanged and not run                    |
...
The existing `signals/{strategy}/{timeframe}/{exchange}.parquet` gains additive `input_versions` and `intraday_confirmation` fields. No new intraday feature dataset is created.

## Metadata Outputs

No new dataset metadata output. Analyzer reads the existing P9 immutable READY/version manifest and does not become a metadata writer.

## Algorithm Feature Outputs

`DIRECT`: session open/close, VWAP, session direction, close-versus-VWAP direction/delta, last-30-minute direction, late-volume share, reconciliation status, and exact intraday version are persisted as confirmed-signal evidence.

## Algorithms Unlocked

`CONFIRMED_TREND_EQUALS_V2_INTRADAY` confirms or suppresses the existing daily candidate without creating direction from neutral.

## Contract Impact

| Area                              | Decision                                                                                                                                                                                                                                     |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Kafka/service-to-service protobuf | Unchanged; existing signal job and notification JSON boundaries are reused. Producer and consumer payload schemas do not change.                                                                                                             |
| Object-storage JSON manifest      | Changed: intraday READY/version manifests are symbol-scoped and their partition includes `symbol`; Analyzer consumes them read-only. Old shared date-level pointers require republishing.                                                    |
| Storage path/dataset ownership    | Changed path to `provider/exchange/trading_date/symbol/trades.parquet`; Ingestor retains `intraday-trades`, while Analyzer becomes an exact-date, exact-symbol consumer. Signal rows gain additive fields.                                   |
| Public Java/Python API            | Internal Analyzer APIs and the shared Python path builder are additive/updated; no public service API changes. Producer and Analyzer consumer use the same shared builder contract.                                                          |
| Configuration/environment         | Existing `intraday-trades` path configuration changes to symbol-level readiness; no new setting. Confirmed scheduler metadata adds `SYNC_INTRADAY_EOD` and `intraday-trades` dependencies. Deployments with old objects must republish them. |

## Repository Guidance Updates

No agent guidance change is required because ownership remains consistent with `AGENTS.md`. Canonical data and signal-flow documentation are updated.

## Verification

Owner-authorized local commands run on 2026-09-12:
...
All listed commands passed. CI, PR/commit, MinIO/provider runtime,
notification-runtime, republishing, deployment, and production verification were
not run and remain required where applicable.

## Acceptance Criteria

- Exact-date, exact-symbol input and no-fallback semantics are enforced.
- Each `(provider, exchange, trading_date, symbol)` partition has an independent READY pointer.
- Former shared date-level objects are republished and are not treated as symbol-ready.
- READY/WARNING are consumable with warning propagation; rejected/missing/invalid input is unavailable.
- Facts and confirmation are deterministic and intraday cannot promote neutral.
- V2 algorithm/input lineage and explanations persist without rewriting V1 rows.
- Scheduler dependencies are explicit and the existing notification route is reused.
- Required documentation is synchronized; local Analyzer and Platform checks pass, while external and runtime evidence remains pending.
